...
class SignInController : UIViewController, FUIAuthDelegate {
    ...
    func authUI(_ authUI: FUIAuth, didSignInWith user: User?, error: Error?) {
        do {
            ...
            guard let email = user.email, let username = user.displayName, let name = user.displayName else {
                print("Could not create user from auth because at least one user property wasn't present.")
                // show something to the user that says there was an error and to try again
                return
            }
            try DataModelManager.createUser(user: SonaUser(email: email, username: username, name: name))
        } catch {
